package frc.robot.lib.pathplanner.pathfinding;
...
public class LocalADStar implements Pathfinder {
    ...
    public void setProblem(
            Pose2d startPose, PathPlannerPath goalPath, Translation2d startVelocity, PathConstraints constraints) {
        requestLock.writeLock().lock();
        GridPosition startPos = findClosestNonObstacle(getGridPos(startPose.getTranslation()), requestObstacles);
        if (startPos != null) {
    ...
    public void setProblem(
            Pose2d startPose, Pose2d goalPose, Translation2d startVelocity, PathConstraints constraints) {
        requestLock.writeLock().lock();
        GridPosition startPos = findClosestNonObstacle(getGridPos(startPose.getTranslation()), requestObstacles);
        if (startPos != null) {
            requestStart = startPos;
        ...
        GridPosition gridPos = findClosestNonObstacle(getGridPos(goalPose.getTranslation()), requestObstacles);
        if (gridPos != null) {
            requestGoal = gridPos;
    ...
    public void setDynamicObstacles(
            List<Pair<Translation2d, Translation2d>> obs, Pose2d currentRobotPose, Translation2d startVelocity) {
        ...
        while (true) {
            try {
                ...
                if (reset || minor) {
                    ...
                    doWork(reset, minor, start, goal, realStart, realGoal, obstacles, constraints, startVelocity);
    ...
    private PathPlannerPath checkCache(GridPosition start, GridPosition goal) {
        ...
        for (int i = -kDistanceGridCells; i <= kDistanceGridCells; ++i) {
            for (int j = -kDistanceGridCells; j <= kDistanceGridCells; ++j) {
                if (i == 0 && j == 0) continue;
                key = new Pair<GridPosition, GridPosition>(new GridPosition(start.x + i, start.y + j), goal);
                ...
                if (result != null) {
                    ...
                    // key.getSecond());
                    double hitDistance =
                            gridPosToTranslation2d(start).getDistance(gridPosToTranslation2d(key.getFirst()));
        ...
        if (closestKey != null) {
            System.out.println("[Cache] Using approx hit " + key.getFirst() + " -> " + key.getSecond());
    ...
    private void doWork(
        ...
        if (doMinor) {
            ...
            List<Waypoint> waypoints =
                    createWaypoints(pathPositions, realStartPose, realGoalPose, obstacles, startVelocity);
            PathPlannerPath path = null;
            if (waypoints.size() >= 2) {
                if (goalPath == null) {
                    path = new PathPlannerPath(
                            waypoints, constraints, null, new GoalEndState(0.0, realGoalPose.getRotation()));
                } else {
                    path = PathPlannerPath.concatenatePaths(
                            waypoints,
                            new IdealStartingState(0.0, realStartPose.getRotation()),
                            new GoalEndState(goalPath.getIdealStartingState().velocity(), realGoalPose.getRotation()),
    ...
    private List<GridPosition> shortcut(List<GridPosition> path, Set<GridPosition> obstacles) {
        ...
        // --- Path Reconstruction ---
        List<GridPosition> shortPath = new LinkedList<>(); // Use LinkedList for efficient prepending
