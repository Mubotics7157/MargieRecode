...
/** A state along the a {@link frc.robot.lib.pathplanner.trajectory.PathPlannerTrajectory} */
public class PathPlannerTrajectoryState implements Interpolatable<PathPlannerTrajectoryState> {
    ...
    /** The {@link frc.robot.lib.pathplanner.trajectory.SwerveModuleTrajectoryState} states for this state */
    protected SwerveModuleTrajectoryState[] moduleStates;

    /** The {@link frc.robot.lib.pathplanner.path.PathConstraints} for this state */
    ...
    public PathPlannerTrajectoryState interpolate(PathPlannerTrajectoryState endVal, double t) {
        ...
        lerpedState.fieldSpeeds = new ChassisSpeeds(
                speeds.getX(),
                speeds.getY(),
                MathUtil.interpolate(fieldSpeeds.omegaRadiansPerSecond, endVal.fieldSpeeds.omegaRadiansPerSecond, t));
        ...
        lerpedState.pose =
                new Pose2d(lerpedXPos, lerpedYPos, pose.getRotation().interpolate(endVal.pose.getRotation(), t));
        lerpedState.feedforwards = feedforwards.interpolate(endVal.feedforwards, t);
        // Approximate out our distance interpoland.
        double lerpedToPrev = lerpedState.pose.getTranslation().getDistance(this.pose.getTranslation());
        double lerpedToNext = lerpedState.pose.getTranslation().getDistance(endVal.pose.getTranslation());
        double s = lerpedToPrev / (lerpedToPrev + lerpedToNext);
        lerpedState.waypointRelativePos = MathUtil.interpolate(waypointRelativePos, endVal.waypointRelativePos, s);
    ...
    public PathPlannerTrajectoryState reverse() {
        ...
        Translation2d reversedSpeeds = new Translation2d(fieldSpeeds.vxMetersPerSecond, fieldSpeeds.vyMetersPerSecond)
                .rotateBy(Rotation2d.k180deg);
        reversed.fieldSpeeds =
                new ChassisSpeeds(reversedSpeeds.getX(), reversedSpeeds.getY(), fieldSpeeds.omegaRadiansPerSecond);
