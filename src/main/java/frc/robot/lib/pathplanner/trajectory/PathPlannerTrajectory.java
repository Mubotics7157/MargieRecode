...
import frc.robot.lib.pathplanner.events.*;
import frc.robot.lib.pathplanner.path.EventMarker;
import frc.robot.lib.pathplanner.path.PathPlannerPath;
import frc.robot.lib.pathplanner.path.PathPoint;
import frc.robot.lib.pathplanner.path.PointTowardsZone;
...
public class PathPlannerTrajectory {
    ...
    private final boolean isStayStoppedTrajectory;

    public boolean isStayStoppedTrajectory() {
        return this.isStayStoppedTrajectory;
    }

    public static PathPlannerTrajectory makeStayInPlaceTrajectory() {
    ...
    private PathPlannerTrajectory() {
        this.states = null;
        this.events = null;
        this.isStayStoppedTrajectory = true;
    }
    ...
    public PathPlannerTrajectory(List<PathPlannerTrajectoryState> states, List<Event> events) {
        this.states = states;
        this.events = events;
        this.isStayStoppedTrajectory = false;
    }
    ...
    public PathPlannerTrajectory(
            PathPlannerPath path, ChassisSpeeds startingSpeeds, Rotation2d startingRotation, RobotConfig config) {
        this.isStayStoppedTrajectory = false;
        if (path.isChoreoPath()) {
            ...
            // Project the starting speeds onto the path, because we can do nothing about components
            // that
            // aren't aligned with it.
            Translation2d startVel =
                    new Translation2d(startingSpeeds.vxMetersPerSecond, startingSpeeds.vyMetersPerSecond);
            Rotation2d startHeading = path.getAllPathPoints()
                    .get(1)
                    .position
                    .minus(path.getAllPathPoints().get(0).position)
                    .getAngle();
            // dot product - If start vel is the wrong direction, best we can do is start at zero
            // speed.
            double speed = startVel.getX() * startHeading.getCos() + startVel.getY() * startHeading.getSin();
            if (speed < 0.0) {
                speed = 0.0;
            }
            startVel = new Translation2d(speed, startHeading);
            startingSpeeds = new ChassisSpeeds(startVel.getX(), startVel.getY(), startingSpeeds.omegaRadiansPerSecond);

            // Set the initial module velocities
            ChassisSpeeds fieldStartingSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(
                    startingSpeeds, states.get(0).pose.getRotation());
            ...
            Translation2d endSpeedTrans =
                    new Translation2d(path.getGoalEndState().velocityMPS(), states.get(states.size() - 1).heading);
            ChassisSpeeds endFieldSpeeds = new ChassisSpeeds(endSpeedTrans.getX(), endSpeedTrans.getY(), 0.0);
            Logger.recordOutput("PathPlanner/EndFieldSpeeds", endFieldSpeeds);
            Logger.recordOutput("PathPlanner/EndSpeedTrans", endSpeedTrans);
            Logger.recordOutput("PathPlanner/EndSpeedTransHeading", states.get(states.size() - 1).heading);

            var endStates = config.toSwerveModuleStates(ChassisSpeeds.fromFieldRelativeSpeeds(
                    endFieldSpeeds, states.get(states.size() - 1).pose.getRotation()));
            for (int m = 0; m < config.numModules; m++) {
                states.get(states.size() - 1).moduleStates[m].speedMetersPerSecond = endStates[m].speedMetersPerSecond;
            }
            ...
            for (int i = 1; i < states.size(); i++) {
                ...
                if (Math.abs(sumV) < 1e-6 || Math.abs(state.deltaPos) < 1e-6) {
                    ...
                    ChassisSpeeds prevRobotSpeeds =
                            ChassisSpeeds.fromFieldRelativeSpeeds(prevState.fieldSpeeds, prevState.pose.getRotation());
                    ChassisSpeeds robotSpeeds =
                            ChassisSpeeds.fromFieldRelativeSpeeds(state.fieldSpeeds, state.pose.getRotation());

                    double chassisAccelX = (robotSpeeds.vxMetersPerSecond - prevRobotSpeeds.vxMetersPerSecond) / dt;
                    double chassisAccelY = (robotSpeeds.vyMetersPerSecond - prevRobotSpeeds.vyMetersPerSecond) / dt;
                    ...
                    // chassisForcesToWheelForceVectors doesn't handle the fact that centripetal
                    // force is
                    // unevenly distributed across wheels proportional
                    // to distance from the instantaneous center of rotation. Handle that part
                    // manually.
                    double centripetalAccelMagnitude = state.curvature
                            * (robotSpeeds.vxMetersPerSecond * robotSpeeds.vxMetersPerSecond
                                    + robotSpeeds.vyMetersPerSecond * robotSpeeds.vyMetersPerSecond);
                    Translation2d centripetalAccelFieldFrame =
                            new Translation2d(centripetalAccelMagnitude, state.heading.rotateBy(Rotation2d.kCCW_90deg));
                    Translation2d centripetalAccelRobotFrame = centripetalAccelFieldFrame.rotateBy(
                            state.pose.getRotation().unaryMinus());
                    Translation2d centripetalForceRobotFrame = centripetalAccelRobotFrame.times(config.massKG);
                    ChassisSpeeds chassisForcesCentripetal = new ChassisSpeeds(
                            centripetalForceRobotFrame.getX(), centripetalForceRobotFrame.getY(), 0.0);
                    Translation2d[] wheelForcesCentripetal =
                            config.chassisForcesToWheelForceVectors(chassisForcesCentripetal);

                    Translation2d totalChassisAccel = new Translation2d(
                            chassisAccelX + centripetalAccelRobotFrame.getX(),
                            chassisAccelY + centripetalAccelRobotFrame.getY());

                    Translation2d icrRobotFrame = Math.abs(state.curvature) > 1e-6
                            ? new Translation2d(1.0 / Math.abs(state.curvature), centripetalForceRobotFrame.getAngle())
                            : Translation2d.kZero;
                    double icrDistance = icrRobotFrame.getDistance(state.pose.getTranslation());
                    ...
                    for (int m = 0; m < config.numModules; m++) {
                        double icrDistModule = config.moduleLocations[m].getDistance(icrRobotFrame);
                        double centripetalRatio = icrDistance > 1e-6 ? icrDistModule / icrDistance : 0.0;
                        // System.out.println("Module " + m);
                        // System.out.println("Curvature " + state.curvature);
                        // System.out.println("icr Dist " + icrDistModule + " (cog " + icrDistance +
                        // ")");
                        // System.out.println("Long forces XY " + wheelForces[m].getX() + " " +
                        // wheelForces[m].getY());
                        // System.out.println("Centrip forces XY " +centripetalRatio *
                        // wheelForcesCentripetal[m].getX() + " " + centripetalRatio *
                        // wheelForcesCentripetal[m].getY());

                        forceXFF[m] = wheelForces[m].getX() + centripetalRatio * wheelForcesCentripetal[m].getX();
                        forceYFF[m] = wheelForces[m].getY() + centripetalRatio * wheelForcesCentripetal[m].getY();

                        // Scale the forces by the delta in normal force induced by weight transfer.
                        double scalingFactor = Math.max(
                                0.0,
                                (config.massKG / config.numModules
                                                - .5
                                                        * totalChassisAccel.getX()
                                                        * config.COGheightMeters
                                                        / config.moduleLocations[m].getX()
                                                - .5
                                                        * totalChassisAccel.getY()
                                                        * config.COGheightMeters
                                                        / config.moduleLocations[m].getY())
                                        / (config.massKG / config.numModules));

                        forceXFF[m] *= scalingFactor;
                        forceYFF[m] *= scalingFactor;

                        // UNUSED BELOW THIS, these do not account for centripetal accel.
                        ...
                        accelFF[m] = (state.moduleStates[m].speedMetersPerSecond
                ...
                while (!unaddedEvents.isEmpty()
                        && Math.abs(unaddedEvents.element().getTimestampSeconds() - prevState.waypointRelativePos)
                                <= Math.abs(
    ...
    private static void generateStates(
        ...
        for (int i = 0; i < path.numPoints(); i++) {
            ...
            double prevToNextDistance = path.getPoint(nextRotationTargetIdx).distanceAlongPath
                    - path.getPoint(prevRotationTargetIdx).distanceAlongPath;
            double t = (path.getPoint(i).distanceAlongPath - path.getPoint(prevRotationTargetIdx).distanceAlongPath)
                    / prevToNextDistance;
            if (prevToNextDistance <= 1e-6) {
            ...
            state.curvature = path.getPoint(i).curvature;
            ...
            if (i != path.numPoints() - 1) {
                ...
                if (headingTranslation.getNorm() <= 1e-6) {
                    state.heading = (i == 0) ? Rotation2d.kZero : states.get(i - 1).heading;
                } else {
            ...
            if (i != 0) {
                state.deltaPos = state.pose
                        .getTranslation()
                        .getDistance(states.get(i - 1).pose.getTranslation());
                state.deltaRot =
                        state.pose.getRotation().minus(states.get(i - 1).pose.getRotation());
            }
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                state.moduleStates[m].fieldPos =
                        state.pose.getTranslation().plus(config.moduleLocations[m].rotateBy(state.pose.getRotation()));

                if (i != 0) {
                    state.moduleStates[m].deltaPos =
                            state.moduleStates[m].fieldPos.getDistance(states.get(i - 1).moduleStates[m].fieldPos);
                }
            }
            ...
            for (int m = 0; m < config.numModules; m++) {
                if (i != states.size() - 1) {
                    Translation2d fieldTranslation =
                            states.get(i + 1).moduleStates[m].fieldPos.minus(states.get(i).moduleStates[m].fieldPos);
                    ...
                    states.get(i).moduleStates[m].angle = states.get(i)
                            .moduleStates[m]
                            .fieldAngle
                            .minus(states.get(i).pose.getRotation());
                } else {
    ...
    private static void forwardAccelPass(List<PathPlannerTrajectoryState> states, RobotConfig config) {
        for (int i = 1; i < states.size() - 1; i++) {
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                // This pass will only be handling acceleration of the robot, meaning that the
                // "torque"
                ...
                Rotation2d angleToModule = state.moduleStates[m]
                        .fieldPos
            ...
            Rotation2d effectiveAccelDirection = state.pose.getRotation().minus(state.heading);
            double lonXYAccelLimit = Math.max(
                    Math.abs(state.constraints.maxXAccelMPSSq() * effectiveAccelDirection.getCos()),
                    Math.abs(state.constraints.maxYAccelMPSSq() * effectiveAccelDirection.getSin()));
            // Rotate so that +x is aligned with heading and +y is aligned with centripetal
            // acceleration.
            double maxAccel = Math.min(state.constraints.maxAccelerationMPSSq(), lonXYAccelLimit);
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                // vf^2 = v0^2 + 2ad
                state.moduleStates[m].speedMetersPerSecond = Math.sqrt(
                        Math.abs(Math.pow(prevState.moduleStates[m].speedMetersPerSecond, 2)
                ...
                if (state.curvature == 0.0) {
                    ...
                    curveRadius = 1.0 / state.curvature;
                }

                // Find the max velocity that would keep the centripetal force under the friction
                // force
                // Fc = M * v^2 / R
                // Also make sure accel limits are obeyed.
                // Ac = v^2 / R
                // v = sqrt(R * Ac)
                if (Double.isFinite(curveRadius)) {
                    double latXYAccelLimit = Math.max(
                            Math.abs(state.constraints.maxXAccelMPSSq() * effectiveAccelDirection.getSin()),
                            Math.abs(state.constraints.maxYAccelMPSSq() * effectiveAccelDirection.getCos()));
                    double maxSafeVel = Math.min(
                            Math.sqrt((config.wheelFrictionForce * Math.abs(curveRadius))
                                    / (config.massKG / config.numModules)),
                            Math.sqrt(Math.abs(curveRadius) * latXYAccelLimit));
                    state.moduleStates[m].speedMetersPerSecond =
                            Math.min(state.moduleStates[m].speedMetersPerSecond, maxSafeVel);
                }
            }

            // Go over the modules again to make sure they take the same amount of time to reach the
            // next
            // state
            double maxDT = 0.0;
            ...
            state.fieldSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(
                    config.toChassisSpeeds(state.moduleStates), state.pose.getRotation());
            state.linearVelocity = Math.hypot(state.fieldSpeeds.vxMetersPerSecond, state.fieldSpeeds.vyMetersPerSecond);
        }
    }
    ...
    private static void reverseAccelPass(List<PathPlannerTrajectoryState> states, RobotConfig config) {
        for (int i = states.size() - 2; i > 0; i--) {
            ...
            for (int m = 0; m < config.numModules; m++) {
                ...
                // This pass will only be handling deceleration of the robot, meaning that the
                // "torque"
                // acting on the module due to friction and other losses will not be fighting the
                // motor
                double lastVelRadPerSec = lastVel / config.moduleConfig.wheelRadiusMeters;
                double currentDraw = Math.min(
                        config.moduleConfig.driveMotor.getCurrent(
                ...
                Translation2d forceVec =
                        new Translation2d(forceAtCarpet, state.moduleStates[m].fieldAngle.plus(Rotation2d.k180deg));
            ...
            // Rotate so that +x is aligned with heading and +y is aligned with centripetal
            // acceleration.
            Translation2d accelLimitsVec = new Translation2d(
                            state.constraints.maxXAccelMPSSq(), state.constraints.maxYAccelMPSSq())
                    .rotateBy(state.pose.getRotation())
                    .rotateBy(state.heading.unaryMinus());
            double maxAccel = Math.min(state.constraints.maxAccelerationMPSSq(), Math.abs(accelLimitsVec.getX()));
            double accel = accelVec.getNorm();
            if (accel > maxAccel) {
                accelVec = accelVec.times(maxAccel / accel);
            }
            ...
            ChassisSpeeds chassisAccel = ChassisSpeeds.fromFieldRelativeSpeeds(
                    new ChassisSpeeds(accelVec.getX(), accelVec.getY(), angularAccel), state.pose.getRotation());
            var accelStates = config.toSwerveModuleStates(chassisAccel);
            for (int m = 0; m < config.numModules; m++) {
                double moduleAcceleration = accelStates[m].speedMetersPerSecond;
                ...
                // Calculate the module velocity at the current state
                // vf^2 = v0^2 + 2ad
                double maxVel = Math.sqrt(Math.abs(Math.pow(nextState.moduleStates[m].speedMetersPerSecond, 2)
            ...
            maxChassisAngVel = Math.min(maxChassisAngVel, Math.abs(state.fieldSpeeds.omegaRadiansPerSecond));
    ...
    private static void desaturateWheelSpeeds(
        ...
        if (Math.abs(maxTranslationSpeed) > 1e-8) {
            translationPct = Math.sqrt(
                            Math.pow(desiredSpeeds.vxMetersPerSecond, 2) + Math.pow(desiredSpeeds.vyMetersPerSecond, 2))
                    / maxTranslationSpeed;
        }
