...
public class PPLibTelemetry {
    private static boolean compMode = true; // Hard code, we never use this!

    private static final DoubleArrayPublisher velPub = NetworkTableInstance.getDefault()
            .getDoubleArrayTopic("/PathPlanner/vel")
            .publish();
    private static final StructPublisher<Pose2d> posePub = NetworkTableInstance.getDefault()
            .getStructTopic("/PathPlanner/currentPose", Pose2d.struct)
            .publish();
    private static final StructArrayPublisher<Pose2d> pathPub = NetworkTableInstance.getDefault()
            .getStructArrayTopic("/PathPlanner/activePath", Pose2d.struct)
            .publish();
    private static final StructPublisher<Pose2d> targetPosePub = NetworkTableInstance.getDefault()
    ...
    private static void ensureHotReloadListenersInitialized() {
        if (hotReloadPathListener == null) {
            hotReloadPathListener = NetworkTableListener.createListener(
                    NetworkTableInstance.getDefault().getStringTopic("/PathPlanner/HotReload/hotReloadPath"),
                    EnumSet.of(NetworkTableEvent.Kind.kValueRemote),
                    PPLibTelemetry::handlePathHotReloadEvent);
        }
        if (hotReloadAutoListener == null) {
            hotReloadAutoListener = NetworkTableListener.createListener(
                    NetworkTableInstance.getDefault().getStringTopic("/PathPlanner/HotReload/hotReloadAuto"),
                    EnumSet.of(NetworkTableEvent.Kind.kValueRemote),
