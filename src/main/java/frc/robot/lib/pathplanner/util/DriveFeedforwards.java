package frc.robot.lib.pathplanner.util;
...
/**
 * Collection of different feedforward values for each drive module. If using swerve, these values will all be in FL,
 * FR, BL, BR order. If using a differential drive, these will be in L, R order.
 *
 * <p>NOTE: If using Choreo paths, all feedforwards but the X and Y component arrays will be filled with zeros.
 *
 ...
 * @param robotRelativeForcesXNewtons X components of robot-relative force vectors for the wheels in newtons. The
 *     magnitude of these vectors will typically be greater than the linear force feedforwards due to friction forces.
 * @param robotRelativeForcesYNewtons X components of robot-relative force vectors for the wheels in newtons. The
 *     magnitude of these vectors will typically be greater than the linear force feedforwards due to friction forces.
 */
public record DriveFeedforwards(
    ...
    /**
     * Collection of different feedforward values for each drive module. If using swerve, these values will all be in
     * FL, FR, BL, BR order. If using a differential drive, these will be in L, R order.
     *
     * <p>NOTE: If using Choreo paths, all feedforwards but the X and Y component arrays will be filled with zeros.
     *
     ...
     * @param robotRelativeForcesX X components of robot-relative force vectors for the wheels. The magnitude of these
     *     vectors will typically be greater than the linear force feedforwards due to friction forces.
     * @param robotRelativeForcesY X components of robot-relative force vectors for the wheels. The magnitude of these
     *     vectors will typically be greater than the linear force feedforwards due to friction forces.
     */
    public DriveFeedforwards(
        ...
        this(
                Arrays.stream(accelerations)
                        .mapToDouble(x -> x.in(MetersPerSecondPerSecond))
                        .toArray(),
                ...
                Arrays.stream(robotRelativeForcesX)
                        .mapToDouble(x -> x.in(Newtons))
                        .toArray(),
                Arrays.stream(robotRelativeForcesY)
                        .mapToDouble(x -> x.in(Newtons))
                        .toArray());
    }
