 ...
 *
 * <p>Takes a prior setpoint, a desired setpoint, and outputs a new setpoint that respects all the kinematic constraints
 * on module rotation and wheel velocity/torque, as well as preventing any forces acting on a module's wheel from
 * exceeding the force of friction.
 */
public class SwerveSetpointGenerator {
    ...
    /**
     * Generate a new setpoint with explicit battery voltage. Note: Do not discretize ChassisSpeeds passed into or
     * returned from this method. This method will discretize the speeds for you.
     *
     ...
     * @param desiredStateRobotRelative The desired state of motion, such as from the driver sticks or a path following
     *     algorithm.
     * @param constraints The arbitrary constraints to respect along with the robot's max capabilities. If this is null,
     *     the generator will only limit setpoints by the robot's max capabilities.
     * @param dt The loop time.
     * @param inputVoltage The input voltage of the drive motor controllers, in volts. This can also be a static nominal
     *     voltage if you do not want the setpoint generator to react to changes in input voltage. If the given voltage
     *     is NaN, it will be assumed to be 12v. The input voltage will be clamped to a minimum of the robot
     *     controller's brownout voltage.
     * @return A Setpoint object that satisfies all the kinematic/friction limits while converging to desiredState
     *     quickly.
     */
    public SwerveSetpoint generateSetpoint(
        ...
        for (int m = 0; m < config.numModules; m++) {
            ...
            prev_vy[m] =
                    prevSetpoint.moduleStates()[m].angle.getSin() * prevSetpoint.moduleStates()[m].speedMetersPerSecond;
            ...
            desired_vx[m] = desiredModuleStates[m].angle.getCos() * desiredModuleStates[m].speedMetersPerSecond;
            desired_vy[m] = desiredModuleStates[m].angle.getSin() * desiredModuleStates[m].speedMetersPerSecond;
            ...
            if (all_modules_should_flip) {
                double required_rotation_rad = Math.abs(prev_heading[m]
                        .unaryMinus()
        ...
        double dx = desiredStateRobotRelative.vxMetersPerSecond - prevSetpoint.robotRelativeSpeeds().vxMetersPerSecond;
        double dy = desiredStateRobotRelative.vyMetersPerSecond - prevSetpoint.robotRelativeSpeeds().vyMetersPerSecond;
        ...
        for (int m = 0; m < config.numModules; m++) {
            double lastVelRadPerSec =
                    prevSetpoint.moduleStates()[m].speedMetersPerSecond / config.moduleConfig.wheelRadiusMeters;
        ...
        double angularAccel =
                (retSpeeds.omegaRadiansPerSecond - prevSetpoint.robotRelativeSpeeds().omegaRadiansPerSecond) / dt;
    ...
    public SwerveSetpoint generateSetpoint(
        ...
        return generateSetpoint(
                prevSetpoint, desiredStateRobotRelative, constraints, dt.in(Seconds), inputVoltage.in(Volts));
    }

    /**
     * Generate a new setpoint. Note: Do not discretize ChassisSpeeds passed into or returned from this method. This
     * method will discretize the speeds for you.
     *
    ...
    public SwerveSetpoint generateSetpoint(
            ...
            double dt) {
        return generateSetpoint(
                prevSetpoint, desiredStateRobotRelative, constraints, dt, RobotController.getInputVoltage());
    }
     ...
     */
    public SwerveSetpoint generateSetpoint(
            final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredStateRobotRelative, Time dt, Voltage inputVoltage) {
    ...
    public SwerveSetpoint generateSetpoint(
        ...
        return generateSetpoint(
                prevSetpoint, desiredStateRobotRelative, null, dt.in(Seconds), RobotController.getBatteryVoltage());
    }
    ...
    private static double findSteeringMaxS(
            double x_0, double y_0, double theta_0, double x_1, double y_1, double theta_1, double max_deviation) {
