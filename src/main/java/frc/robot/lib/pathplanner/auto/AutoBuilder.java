package frc.robot.lib.pathplanner.auto;
...
import frc.robot.lib.pathplanner.commands.FollowPathCommand;
import frc.robot.lib.pathplanner.commands.PathPlannerAuto;
import frc.robot.lib.pathplanner.commands.PathfindingCommand;
import frc.robot.lib.pathplanner.config.RobotConfig;
import frc.robot.lib.pathplanner.path.IPathCallback;
...
public class AutoBuilder {
     ...
     * @param robotRelativeSpeedsSupplier a supplier for the robot's current robot relative chassis speeds
     * @param output Output function that accepts robot-relative ChassisSpeeds and feedforwards for each drive motor. If
     *     using swerve, these feedforwards will be in FL, FR, BL, BR order. If using a differential drive, they will be
     *     in L, R order.
     *     <p>NOTE: These feedforwards are assuming unoptimized module states. When you optimize your module states, you
     *     will need to reverse the feedforwards for modules that have been flipped
     * @param controller Path following controller that will be used to follow paths
     * @param robotConfig The robot configuration
     * @param shouldFlipPath Supplier that determines if paths should be flipped to the other side of the field. This
    ...
    public static void configure(
        ...
        AutoBuilder.pathFollowingCommandBuilder = (path) -> new FollowPathCommand(
                path,
                poseSupplier,
        ...
        AutoBuilder.pathfindToPoseCommandBuilder = (pose, constraints, goalEndVel) -> new PathfindingCommand(
                pose,
                constraints,
                goalEndVel,
                poseSupplier,
        ...
        AutoBuilder.pathfindThenFollowPathCommandBuilder =
                (targetPath, constraints, pathCallback) -> new PathfindingCommand(
                        targetPath,
                        pathCallback,
                        constraints,
                        poseSupplier,
    ...
    /**
     * Configures the AutoBuilder with custom path following command builder. Building pathfinding commands is not
     * supported if using a custom command builder. Custom path following commands will not have the path flipped for
     * them, and event markers will not be triggered automatically.
     *
     * @param pathFollowingCommandBuilder a function that builds a command to follow a given path
     * @param poseSupplier a supplier for the robot's current pose
     * @param resetPose a consumer for resetting the robot's pose
     * @param shouldFlipPose Supplier that determines if the starting pose should be flipped to the other side of the
     *     field. This will maintain a global blue alliance origin. NOTE: paths will not be flipped when configured with
     *     a custom path following command. Flipping the paths must be handled in your command.
    ...
    /**
     * Build a command to pathfind to a given pose. If not using a holonomic drivetrain, the pose rotation and rotation
     * delay distance will have no effect.
     *
    ...
    /**
     * Build a command to pathfind to a given pose. If not using a holonomic drivetrain, the pose rotation will have no
     * effect.
     *
     * @param pose The pose to pathfind to
     * @param constraints The constraints to use while pathfinding
    ...
    /**
     * Build a command to pathfind to a given pose that will be flipped based on the value of the path flipping supplier
     * when this command is run. If not using a holonomic drivetrain, the pose rotation and rotation delay distance will
     * have no effect.
     *
    ...
    /**
     * Build a command to pathfind to a given pose that will also trigger an additional command when the remaining time
     * on the path is less than or equal to triggerTimeBeforeEnd seconds.
     *
     * @param pose The pose to pathfind to.
     * @param constraints The constraints to use while pathfinding.
     * @param goalEndVelocity The goal end velocity of the robot when reaching the target pose.
     * @param additionalCommand The extra command to schedule near the end of the path.
     * @param triggerTimeBeforeEnd The time (in seconds) before the end of the path when the additionalCommand should
     *     run.
     * @return A command that runs the pathfinding and triggers the additional command at the right time.
     */
    public static Command pathfindToPoseWithAdditionalCommand(
            ...
            boolean flipPath) {
        Command pathfindCommand = flipPath
                ? pathfindToPoseFlipped(pose, constraints, goalEndVelocity)
                : pathfindToPose(pose, constraints, goalEndVelocity);

        if (!(pathfindCommand instanceof PathfindingCommand)) {
            throw new IllegalArgumentException("The returned command is not an instance of PathfindingCommand");
        }
        ...
        return new edu.wpi.first.wpilibj2.command.ParallelCommandGroup(
                pathfindCommand,
                new WaitUntilCommand(() -> !pfCommand.getTrajectoryIsNull()
                                && pfCommand.getRemainingTime() <= triggerTimeBeforeEnd)
                        .andThen(additionalCommand));
    }

    /**
     * Build a command to pathfind to a given path, then follow that path. If not using a holonomic drivetrain, the pose
    ...
    public static Command pathfindThenFollowPath(
            PathPlannerPath goalPath, PathConstraints pathfindingConstraints, IPathCallback pathCallback) {
        if (!isPathfindingConfigured()) {
            throw new AutoBuilderException(
                    "Auto builder was used to build a pathfinding command before being configured");
        }

        return pathfindThenFollowPathCommandBuilder.apply(goalPath, pathfindingConstraints, pathCallback);
    }

    /**
     * Create and populate a sendable chooser with all PathPlannerAutos in the project. The default option will be
     * Commands.none()
     *
     ...
     *
     * @param defaultAutoName The name of the auto that should be the default option. If this is an empty string, or if
     *     an auto with the given name does not exist, the default option will be Commands.none()
     * @return SendableChooser populated with all autos
     */
     ...
     * Commands.none()
     *
     * @param optionsModifier A lambda function that can be used to modify the options before they go into the
     *     AutoChooser
     * @return SendableChooser populated with all autos
     */
