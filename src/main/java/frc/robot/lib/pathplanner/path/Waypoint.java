...
import frc.robot.lib.pathplanner.util.JSONUtil;
...
public record Waypoint(Translation2d prevControl, Translation2d anchor, Translation2d nextControl) {
    ...
    private static final double AUTO_CONTROL_VELOCITY_SQR_FACTOR = 0.1;
    ...
    /**
     * Create a waypoint with auto calculated control points based on the positions of adjacent waypoints. This is used
     * internally, and you probably shouldn't use this.
     *
     * @param anchor The anchor point of the waypoint to create
     * @param heading The heading of this waypoint
     * @param prevAnchor The position of the previous anchor point. This can be null for the start point
    ...
    public static Waypoint autoControlPoints(
            Translation2d anchor, Rotation2d heading, Translation2d prevAnchor, Translation2d nextAnchor) {
    ...
    public static Waypoint autoControlPoints(
        ...
        if (prevAnchor != null) {
            double d = anchor.getDistance(prevAnchor) * distanceFactor;
            prevControl = anchor.minus(new Translation2d(d, heading));
        }

        if (nextAnchor != null) {
            double d = anchor.getDistance(nextAnchor) * distanceFactor;
            nextControl = anchor.plus(new Translation2d(d, heading));
        }
    ...
    public static Waypoint autoControlFirstPoint(
            Translation2d anchor,
            Rotation2d heading,
            Translation2d prevAnchor,
            Translation2d nextAnchor,
        ...
        Translation2d velocitySqr =
                new Translation2d(startVelocity.getNorm() * startVelocity.getNorm(), startVelocity.getAngle());

        if (prevAnchor != null) {
            prevControl = anchor.minus(velocitySqr.times(AUTO_CONTROL_VELOCITY_SQR_FACTOR));
        }

        if (nextAnchor != null) {
            nextControl = anchor.plus(velocitySqr.times(AUTO_CONTROL_VELOCITY_SQR_FACTOR));
        }
