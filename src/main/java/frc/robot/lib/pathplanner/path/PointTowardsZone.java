 ...
 * @param rotationOffset A rotation offset to add on top of the angle to the target position. For example, if you want
 *     the robot to point away from the target position, use a rotation offset of 180 degrees
...
public record PointTowardsZone(
        String name, Translation2d targetPosition, Rotation2d rotationOffset, double minPosition, double maxPosition) {
    /**
    ...
    public PointTowardsZone(
            String name, Translation2d targetPosition, double minWaypointRelativePos, double maxWaypointRelativePos) {
    ...
    public PointTowardsZone flip() {
        return new PointTowardsZone(
                name, FlippingUtil.flipFieldPosition(targetPosition), rotationOffset, minPosition, maxPosition);
    }
}
